    ...
    Constraint("constraint.nominalValueOfShares") {
      value =>
        val errors = Try {BigDecimal(value)} match {
          case Success(result) =>
            val decimal = if (!(result.scale == 0)) Seq(ValidationError("validation.error.nominalValueOfShares.decimalPlaces")) else Seq()
            val size = if (result.precision > 13) Seq(ValidationError("validation.error.nominalValueOfShares.size")) else Seq()
            val negative = if (result < 0) Seq(ValidationError("validation.error.nominalValueOfShares.negative")) else Seq()
