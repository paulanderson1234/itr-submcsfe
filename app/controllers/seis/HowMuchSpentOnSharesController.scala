      ...
      implicit user =>
        implicit request =>
          s4lConnector.fetchAndGetFormData[HowMuchSpentOnSharesModel](KeystoreKeys.howMuchSpentOnShares).map {
            case Some(data) => Ok(views.html.seis.investors.HowMuchSpentOnShares(howMuchSpentOnSharesForm.fill(data)))
            case None => Ok(views.html.seis.investors.HowMuchSpentOnShares(howMuchSpentOnSharesForm))
          }
    }
  }
      ...
      implicit user =>
        implicit request =>
          ...
          val failure: Form[HowMuchSpentOnSharesModel] => Future[Result] = { form =>
            Future.successful(BadRequest(views.html.seis.investors.HowMuchSpentOnShares(form)))
          }
