...
import views.html.seis.investors.DeletePreviousShareHolder
...
trait DeletePreviousShareHolderController extends FrontendController with AuthorisedAndEnrolledForTAVC with FeatureSwitch with ControllerHelpers{
  ...
  def submit(investorProcessingId: Int, id: Int): Action[AnyContent] = featureSwitch(applicationConfig.seisFlowEnabled) {
    AuthorisedAndEnrolled.async { implicit user =>
      implicit request =>
        PreviousInvestorShareHoldersHelper.removePreviousShareHolders(s4lConnector, investorProcessingId, id).map{
          data => Redirect(controllers.seis.routes.PreviousShareHoldingsReviewController.show(data.processingId.get))
        }
    }
  }
}
