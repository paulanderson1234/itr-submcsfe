...
trait ReviewInvestorDetailsController extends FrontendController with AuthorisedAndEnrolledForTAVC with ControllerHelpers {
    ...
    s4lConnector.fetchAndGetFormData[Vector[InvestorDetailsModel]](KeystoreKeys.investorDetails).map { vector =>
      redirectEisNoInvestors(vector) { data =>
        ...
        redirectEisInvalidInvestor(itemToUpdateIndex) { index =>
          Ok(views.html.eis.investors.ReviewInvestorDetails(retrieveInvestorData(index, data)(model => Option(model)).get))
        }
      }
    }
  }

  def change(actionType: String, id: Int): Action[AnyContent] = AuthorisedAndEnrolled.async { implicit user =>
    implicit request =>
      actionType match {
