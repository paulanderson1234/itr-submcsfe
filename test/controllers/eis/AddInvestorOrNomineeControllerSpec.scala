...
class AddInvestorOrNomineeControllerSpec extends BaseSpec {
  ...
  lazy val validBackLink = controllers.eis.routes.UsedInvestmentReasonBeforeController.show().toString
      ...
      "there is no 'back link' present" in {
        ...
        showWithSessionAndAuth(TestController.show(None))(
          result => {
            ...
            redirectLocation(result) shouldBe Some(controllers.eis.routes.ShareDescriptionController.show().url)
          }
        )
      }
    }
      ...
      submitWithSessionAndAuth(TestController.submit,formInput)(
        result => {
          ...
          redirectLocation(result) shouldBe
            Some(controllers.eis.routes.CompanyOrIndividualController.show(listOfInvestorsComplete.head.processingId.get + 1).url)
        }
      )
    }
  }
    ...
    "redirect to the correct page if an investor and the form 'was' previously populated and had a processing id" in {
      ...
      submitWithSessionAndAuth(TestController.submit,formInput:_*)(
        result => {
          status(result) shouldBe SEE_OTHER
          redirectLocation(result) shouldBe
            Some(controllers.eis.routes.CompanyOrIndividualController.show(listOfInvestorsComplete.head.processingId.get).url)
        }
      )
    }
  }
