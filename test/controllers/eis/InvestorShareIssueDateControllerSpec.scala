...
class InvestorShareIssueDateControllerSpec extends BaseSpec {
  ...
  lazy val backUrl = Some(controllers.eis.routes.PreviousShareHoldingNominalValueController.show(2, 1).url)
      ...
      submitWithSessionAndAuth(controller.submit(Some(2)), formInput:_*)(
        result => {
          ...
          redirectLocation(result) shouldBe
            Some(controllers.eis.routes.NumberOfPreviouslyIssuedSharesController.show(listOfInvestorsComplete.head.processingId.get,
