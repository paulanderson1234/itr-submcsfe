...
class CompanyOrIndividualControllerSpec extends BaseSpec {
  ...
  lazy val validBackLink = controllers.eis.routes.AddInvestorOrNomineeController.show().url

  lazy val listOfInvestorsIncompleteCompanyOrIndividual =  Vector(validModelWithPrevShareHoldings.copy(companyOrIndividualModel = None))
      ...
      submitWithSessionAndAuth(TestController.submit(Some("Investor")),formInput)(
        result => {
          ...
          redirectLocation(result) shouldBe
            Some(controllers.eis.routes.CompanyDetailsController.show(listOfInvestorsComplete.head.processingId.get).url)
        }
      )
    }
  }
    ...
    "redirect to the CompanyOrIndividual page if a nominee and the form 'was not' previously populated" in {
      ...
      submitWithSessionAndAuth(TestController.submit(Some("Investor")), formInput)(
        result => {
          status(result) shouldBe SEE_OTHER
          redirectLocation(result) shouldBe
            Some(controllers.eis.routes.IndividualDetailsController.show(listOfInvestorsComplete.head.processingId.get).url)
        }
      )
    }
  }
