...
class IsExistingShareHolderControllerSpec extends BaseSpec with DateFormatter{
  ...
  lazy val backUrl = Some(controllers.eis.routes.HowMuchSpentOnSharesController.show(1).url)
      ...
      "redirect to the 'AddInvestorOrNominee' page" when {
        "a 'backlink' is defined, an 'investor details list' is retrieved but a 'company or individual' is not provided" in {
          ...
          showWithSessionAndAuth(controller.show(2))(
            result => {
              status(result) shouldBe SEE_OTHER
              redirectLocation(result) shouldBe Some(controllers.eis.routes.CompanyOrIndividualController.show(2).url)
            }
          )
        }
      }
        ...
        submitWithSessionAndAuth(controller.submit(Some(Constants.typeCompany)),formInput)(
          result => {
            ...
            redirectLocation(result) shouldBe
              Some(controllers.eis.routes.PreviousShareHoldingDescriptionController.show(listOfInvestorsComplete.head.processingId.get).url)
          }
        )
      }
      ...
      "redirect to the investor review page if the form 'was not' previously populated with a 'No' response" in {
        ...
        submitWithSessionAndAuth(controller.submit(Some(Constants.typeCompany)),formInput)(
          result => {
            status(result) shouldBe SEE_OTHER
            redirectLocation(result) shouldBe
              Some(controllers.eis.routes.ReviewInvestorDetailsController.show(listOfInvestorsComplete.head.processingId.get).url)
          }
        )
      }
    }
