...
class ShareDescriptionControllerSpec extends BaseSpec {
  ...
  lazy val validBackLink = controllers.eis.routes.HadOtherInvestmentsController.show().toString
    ...
    "redirect to the 'HadOtherInvestments' page when no valid back link is present" in {
      ...
      showWithSessionAndAuth(TestController.show())(
        result => {
          ...
          redirectLocation(result) shouldBe Some(controllers.eis.routes.HadOtherInvestmentsController.show().toString)
        }
      )
    }
    "return a 200 when something is fetched from keystore when a valid back and share issue date link are present" in {
