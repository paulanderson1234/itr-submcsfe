...
class TAVCAuthEnrolledSpec extends BaseSpec {
  ...
  "Government Gateway Provider" should {
    "handle a session timeout with a redirect" in {
      ...
      redirectLocation(timeoutHandler) shouldBe Some("/investment-tax-relief-cs/session-timeout")
    }
  }
  ...
  "Calling authenticated async action with no login session" should {
    "result in a redirect to login" in {
      ...
      redirectLocation(result) shouldBe Some(s"/gg/sign-in?continue=${URLEncoder.encode(MockConfig.introductionUrl)}&origin=investment-tax-relief-submission-cs-frontend&accountType=organisation")
    }
  }
