...
import controllers.eisseis.HadPreviousRFIController
...
class HadPreviousRFISpec extends ViewSpec {
  ...
  "Verify that the hadPreviousRFI page contains the correct elements " +
    ...
    val document : Document = {
      setupMocks(Some(controllers.eisseis.routes.ProposedInvestmentController.show().url), Some(hadPreviousRFIModelYes))
  ...
  "Verify that hadPreviousRFI page contains the correct elements when an empty model " +
    ...
    val document : Document = {
      setupMocks(Some(controllers.eisseis.routes.ProposedInvestmentController.show().url))
      val result = TestController.show.apply(authorisedFakeRequest)
      Jsoup.parse(contentAsString(result))
    }
    document.body.getElementById("back-link").attr("href") shouldEqual controllers.eisseis.routes.ProposedInvestmentController.show().url
