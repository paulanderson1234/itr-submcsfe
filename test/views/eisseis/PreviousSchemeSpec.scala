...
import controllers.eisseis.PreviousSchemeController
  ...
  "The Previous Scheme page" should {
    ...
    "Verify that the page contains the correct elements for a new scheme model and back link" in new Setup {
      val document: Document = {
        setupMocks(Some(previousSchemeVectorList),Some(controllers.eisseis.routes.ReviewPreviousSchemesController.show().url))
    ...
    "Verify the page contains the correct elements for an exiting scheme model and changed back link" in new Setup {
      val document: Document = {
        setupMocks(Some(previousSchemeVectorList), Some(controllers.eisseis.routes.HadPreviousRFIController.show().url))
