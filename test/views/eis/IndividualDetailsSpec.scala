...
class IndividualDetailsSpec extends ViewSpec with FakeRequestHelper {
  ...
  val backUrl = controllers.eis.routes.CompanyOrIndividualController.show(1).url
  ...
  "IndividualDetailsView" when {
    ...
    "Supplied with no errors" should {
      ...
      "have a link to the number of shares page" in {
        document.select("article a").first().attr("href") shouldBe controllers.eis.routes.CompanyOrIndividualController.show(1).url
      }
      ...
      "have a form" which {
        ...
        "has an action to the correct route" in {
          form.attr("action") shouldBe controllers.eis.routes.IndividualDetailsController.submit().url
        }
    ...
    "supplied with some errors"  should {
          lazy val view = views.html.eis.investors.IndividualDetails(
