  ...
  "The Research Start Date page" when {
    ...
    "no error occurs" should {
      ...
      "have the correct back link url" in {
        document.select("a.back-link").attr("href") shouldBe  controllers.eis.routes.QualifyBusinessActivityController.show().url
      }
    ...
    "an error occurs" should {
      ...
      "have a form posting to the correct route" in {
        document.select("form").attr("action") shouldBe controllers.eis.routes.ResearchStartDateController.submit().url
      }
