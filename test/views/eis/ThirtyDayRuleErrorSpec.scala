...
class ThirtyDayRuleErrorSpec extends ViewSpec {
 
  "The ThirtyDayRuleError page" should {
    "contain the expected elements" in {
      val page = ThirtyDayRuleError()(fakeRequest, applicationMessages)
      ...
      document.body.getElementById("warning-description").text() shouldEqual Messages("page.ThirtyDayRuleWarning.description")
      ...
      document.body.getElementById("change-answers").attr("href") shouldEqual routes.ThirtyDayRuleController.show().url
      document.body.getElementById("back-link").attr("href") shouldEqual routes.ThirtyDayRuleController.show().url
