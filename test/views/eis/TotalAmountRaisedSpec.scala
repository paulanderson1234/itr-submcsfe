...
class TotalAmountRaisedSpec extends ViewSpec {
  ...
  val page = (form: Form[TotalAmountRaisedModel]) =>
    ...
    "contain the correct elements for a GET when a valid TotalAmountRaisedModel is loaded" in new Setup {
      ...
      document.body.getElementById("back-link").attr("href") shouldEqual routes.NumberOfSharesController.show().url
