...
import views.html.eis.investors.InvestorShareIssueDate
...
class InvestorShareIssueDateSpec extends ViewSpec {
  ...
  val backUrl = controllers.eis.routes.PreviousShareHoldingNominalValueController.show(1, 1).url
    ...
    "not supplied with form errors" should {
      ...
      "have a form posting to the correct route" in {
        document.select("form").attr("action") shouldBe
          controllers.eis.routes.InvestorShareIssueDateController.submit(Some(1)).url
      }
