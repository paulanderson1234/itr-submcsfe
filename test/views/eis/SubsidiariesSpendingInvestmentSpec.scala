...
class SubsidiariesSpendingInvestmentSpec extends ViewSpec {
  ...
  "The SubsidiariesSpendingInvestment Page" +
    ...
    val document: Document = {
      setupMocks(Some(subsidiariesSpendingInvestmentModelYes), Some(controllers.eis.routes.TotalAmountRaisedController.show().url))
  ...
  "Verify that the correct elements are loaded when navigating from PreviousBeforeDOFCS page" in new Setup {
    val document: Document = {
      setupMocks(Some(subsidiariesSpendingInvestmentModelYes), Some(controllers.eis.routes.PreviousBeforeDOFCSController.show().url))
      val result = TestController.show.apply(authorisedFakeRequest)
      Jsoup.parse(contentAsString(result))
    }
  ...
  "Verify that the correct elements are loaded when navigating from NewProduct page" in new Setup {
    val document: Document = {
      setupMocks(Some(subsidiariesSpendingInvestmentModelYes), Some(controllers.eis.routes.NewProductController.show().url))
      val result = TestController.show.apply(authorisedFakeRequest)
      Jsoup.parse(contentAsString(result))
    }
