...
class WasAnyValueReceivedViewSpec extends ViewSpec with FakeRequestHelper {
  ...
  "Was any value received view" when {
    ...
    "no errors are present" should {
      lazy val doc = Jsoup.parse(views.html.seis.investors.WasAnyValueReceived(wasAnyValueReceivedForm).body)
      ...
      "have a back link" which {
        ...
        "links to the review investors page" in {
          backLink.attr("href") shouldBe controllers.seis.routes.ReviewAllInvestorsController.show().url
        }
      }
      ...
      "have a form element" which {
        ...
        "has an action of the page's submit route" in {
          form.attr("action") shouldBe "/investment-tax-relief-cs/seis/was-any-value-received"
        }
