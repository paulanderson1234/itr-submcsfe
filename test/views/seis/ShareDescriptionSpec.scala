    ...
    "show the correct elements" when {
      "there is no share description model" in {
        val page = ShareDescription(shareDescriptionForm,
          controllers.seis.routes.HadOtherInvestmentsController.show().toString, shareIssueDate)(authorisedFakeRequest, applicationMessages)
        ...
        document.getElementById("back-link").attr("href") shouldBe controllers.seis.routes.HadOtherInvestmentsController.show().url
      ...
      "Verify that the Share Description page contains the correct elements when an invalid ShareDescriptionModel is passed" in {
        val page = ShareDescription(shareDescriptionForm.bind(Map("descriptionTextArea" -> "")),
          controllers.seis.routes.ReviewPreviousSchemesController.show().toString, shareIssueDate)(authorisedFakeRequest, applicationMessages)
        val document = Jsoup.parse(page.body)

        document.title() shouldBe Messages("page.shares.shareDescription.title", shareIssueDate)
        document.getElementById("back-link").attr("href") shouldBe controllers.seis.routes.ReviewPreviousSchemesController.show().url
