  ...
  "NumberOfShares view" when {
    ...
    "not supplied with form errors" should {
      ...
      "have the correct back link url" in {
        document.select("a.back-link").attr("href") shouldBe "/investment-tax-relief-cs/seis/share-description"
      }

      "have the progress details" in {
        document.select("article span").first().text shouldBe Messages("common.section.progress.share.details.three")
      }
