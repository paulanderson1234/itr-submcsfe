...
class SeventyPercentSpentSpec extends ViewSpec {
  ...
  def setupMocks(seventyPercentSpentModel: Option[SeventyPercentSpentModel] = None): Unit =
    when(mockS4lConnector.fetchAndGetFormData[SeventyPercentSpentModel](Matchers.eq(KeystoreKeys.seventyPercentSpent))
      (Matchers.any(), Matchers.any(),Matchers.any())).thenReturn(Future.successful(seventyPercentSpentModel))
  ...
  "The Is this the first trade your company has carried out page" should {
    ...
    "Verify that the Is First Trade page contains the correct elements when a valid IsFirstTRadeModel is passed from keystore" in new SEISSetup {
      ...
      document.body.getElementById("back-link").attr("href") shouldEqual controllers.seis.routes.ResearchStartDateController.show().url
