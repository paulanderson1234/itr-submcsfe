...
class IsExistingShareHolderFormSpec extends UnitSpec with OneAppPerSuite{

  private def bindSuccess(request: FakeRequest[AnyContentAsFormUrlEncoded]) = {
    IsExistingShareHolderForm.isExistingShareHolderForm.bindFromRequest()(request).fold(
      formWithErrors => None,
  ...
  val isExistingShareHolderJson = """{"isExistingShareHolder":"Yes"}"""
  val isExistingShareHolderModel = IsExistingShareHolderModel("Yes")
  ...
  "The Existing Share Holder Form" should {
    "Return an error if no radio button is selected" in {
      ...
      bindWithError(request) match {
        case Some(err) => {
          err.key shouldBe "isExistingShareHolder"
  ...
  "The Is Existing Share Holder Form" should {
    "not return an error if the 'Yes' option is selected" in {
      val request = FakeRequest("GET", "/").withFormUrlEncodedBody(
        "isExistingShareHolder" -> Constants.StandardRadioButtonYesValue
      )
      bindWithError(request) match {
        case Some(err) => {
  ...
  "The Is Existing Share Holder Form model" should {
    "load convert to JSON successfully" in {
      ...
      val isExistingShareHolder= Json.toJson(isExistingShareHolderModel).toString()
      isExistingShareHolder shouldBe isExistingShareHolderJson
  ...
  "The Is Existing Share Holder Form model" should {
    "call apply correctly on the model" in {
      implicit val formats = Json.format[IsExistingShareHolderModel]
      val isExistingShareHolderForm =IsExistingShareHolderForm.isExistingShareHolderForm.fill(isExistingShareHolderModel)
      isExistingShareHolderForm.get.isExistingShareHolder shouldBe Constants.StandardRadioButtonYesValue
    }
    ...
    "call unapply successfully to create expected Json" in {
      implicit val formats = Json.format[IsExistingShareHolderModel]
      val isExistingShareHolderForm = IsExistingShareHolderForm.isExistingShareHolderForm.fill(isExistingShareHolderModel)
      val formJson = Json.toJson(isExistingShareHolderForm.get).toString()
      formJson shouldBe isExistingShareHolderJson
    }
  }
}
